...
**v1.3.3 (2026-03-29)**
- 💾 完整备份：JSON 导出升级为完整备份，新增活动簿数据导出与导入恢复
- ⚙️ 默认录入策略：单条与批量新增统一读取默认「收礼 / 送礼」设置
- 🪟 iOS 未签名 IPA：补充 GitHub Actions 构建与 Release 上传链路（该工作流现已停用，改为本地手动构建）
- 🐛 修复统计页年份筛选溢出、不支持平台的应用内更新提示，以及 Android / Windows 发版构建失败
- ♻️ 服务与计算逻辑拆分，活动簿记录数改为批量统计
