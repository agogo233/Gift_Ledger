...
### 修复
- 🔧 **编辑记录**：编辑礼金记录时保留关联记录、还礼状态、提醒次数与提醒时间，避免被重置为默认值；编辑宾客时保留电话与备注
- 🔍 **记录筛选**：记录页搜索与分类切换时保留收礼 / 送礼筛选条件
- 🌐 **Web 存储**：修复还礼状态、提醒次数与关联更新在记录缺失时抛异常的问题；最近记录排序对齐原生实现
- 🔐 **安全锁定**：PIN 与密保答案失败计数增加超时窗口重置，长时间未操作后重新计数，避免遗留计数导致误锁；剩余次数提示同步该窗口
- 🗑️ **代码清理**：移除未引用的 Web 存储旧实现

## [1.3.3] - 2026-03-29

### 新增
- 💾 **完整备份**：JSON 导出升级为完整备份，新增活动簿数据导出与导入恢复
- ⚙️ **默认录入策略**：单条新增与批量新增统一读取默认“收礼 / 送礼”设置
- 🧪 **关键流程测试**：新增覆盖主导航、新增记录、记录列表、待处理与统计筛选的高层流程测试

### 修复
- 🔐 **安全锁定**：PIN 与密保答案增加分级锁定与独立失败控制，减少暴力尝试风险
...
- 🎛️ **交互统一**：统一列表侧滑动作组件与部分页面动画开关，提升交互一致性
- ⚡ **活动簿性能**：活动簿记录数改为批量统计，减少逐条查询开销

## [1.3.2] - 2026-03-26

### 新增
- ✨ **备注外显**：首页与列表卡片展示备注摘要，补齐自定义内容外显
- ✨ **备注输入优化**：添加记录页支持多行备注、字数限制与输入反馈
- ✨ **更新提示整合**：设置页 / 关于页整合更新入口、红点提示与版本忽略

### 修复
- 🐛 **安装流程**：修复取消安装后二次拉起的问题
- 🐛 **更新链路**：修复设置页状态同步与更新提示展示不一致问题
- 🩹 **Windows 发布**：修复 Inno Setup 中文语言文件缺失导致的安装器构建失败

### 优化
- ♻️ **发布元数据**：统一 Android / Windows 产物命名与版本规则
- 📦 **更新清单**：自动生成并回写 `releases/update-manifest.json`
- 📝 **发布文档**：补齐版本规则、Release Notes 与 README 版本信息
